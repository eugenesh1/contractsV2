...
@pytest.fixture(scope="module")
def wvbzrxProxy(accounts, VBZRXWrapper, VBZRXWrapperProxy):
    impl = VBZRXWrapper.deploy({"from": accounts[0]})
    return VBZRXWrapperProxy.deploy(impl, bytes(0), {'from': accounts[0]})
...
def test_mainflow(requireMainnetFork, bzrx, vbzrx, chain, accounts, wrapper):
    ...
    assert wrapper.claimable(account1, {"from": account1}) == 0

    chain.mine()
    assert wrapper.claimable(account1, {"from": account1}) > 0

    tx3 = wrapper.transfer(account2, depositAmount, {"from": account1})
    assert wrapper.balanceOf(account1, {"from": account1}) == 0
    assert wrapper.balanceOf(account2, {"from": account2}) == INITIAL_LP_TOKEN_ACCOUNT_AMOUNT
    assert wrapper.claimable(account1, {"from": account1}) == 0
    assert wrapper.claimable(account2, {"from": account2}) > 0
    assert vbzrx.vestedBalanceOf(account2, {"from": account2}) == 0
    assert vbzrx.vestedBalanceOf(account1, {"from": account1}) == 0
    ...
    claimable2 = wrapper.claimable(account2, {"from": account2})
    assert claimable2 > 0
    chain.mine()
    assert wrapper.claimable(account2, {"from": account2}) == claimable2
    wrapper.claim({"from": account2})
    assert vbzrx.vestedBalanceOf(account2) > 0
    assert wrapper.claimable(account2, {"from": account2}) == 0
    assert vbzrx.vestedBalanceOf(account2) > 0
...
def test_mainflowChangeProxy(requireMainnetFork, bzrx, vbzrx, chain, accounts, wrapper, VBZRXWrapper, wvbzrxProxy):
    ...
    wvbzrxProxy.upgradeTo(impl, {"from": adminAccount})

    with reverts("TransparentUpgradeableProxy: admin cannot fallback to proxy target"):
        wrapper.claimable(account1, {"from": accounts[0]})
...
def test_exit(requireMainnetFork, bzrx, vbzrx, chain, accounts, wrapper):
    ...
    assert wrapper.balanceOf(account1, {"from": account1}) == INITIAL_LP_TOKEN_ACCOUNT_AMOUNT
    chain.mine()
...
def test_transfer1(requireMainnetFork, bzrx, vbzrx, chain, accounts, wrapper):
    ...
    chain.mine()
    assert abs(
        wrapper.claimable(account2, {"from": account1}) - wrapper.claimable(account1, {"from": account1})) / 1e18 < 5e-6
...
def test_transfer2(requireMainnetFork, bzrx, vbzrx, chain, accounts, wrapper):
    ...
    assert abs(wrapper.claimable(account2, {"from": account2}) - wrapper.claimable(account1, {"from": account1})) < 2
...
def test_multiuser_withdraw1(requireMainnetFork, bzrx, vbzrx, chain, accounts, wrapper):
    for i in [1, 3, 5, 7]:
        ...
        assert wrapper.claimable(account1, {"from": account1}) > 0
        assert wrapper.claimable(account2, {"from": account2}) > 0
        wrapper.withdraw(wrapper.balanceOf(account2, {"from": account2}), {"from": account2})
        assert wrapper.claimable(account2, {"from": account2}) > 0
        wrapper.withdraw(wrapper.balanceOf(account1, {"from": account1}), {"from": account1})
        assert wrapper.claimable(account1, {"from": account1}) > 0
